...
- **Brief format:** header shows `conviction ±X.XX (STRONG≥0.50/GOOD≥0.35/MODERATE≥0.15/WEAK)` — conviction = combined_score ∈ [−1,+1], NOT a percentage. Raw RSI hidden (kept out of header AND the LLM prose). Each pick shows ATR-based levels: `Entry ~close · Stop −1 ATR · T1 +2 ATR · T2 +3 ATR` (same ATR basis the tracker grades WIN/LOSS on, so displayed levels match training labels).
