...
- **Options engine (BS-mode backtest + 5% half-spread):** naive signal→long-option conversion is NEGATIVE after costs on most cells (SPY-30 PF 0.62, QQQ-15 0.76, SPY-60 0.38). Keep paper-only; the engine's IV/event gates + ML gate must prove edge first. Paper closes DO persist (status key is `CLOSED`, uppercase — a lowercase check misreads it as zero). Real leak found in the first 7 closes: hourly management let SLs blow through to -56%..-70% instead of -50% → manage cadence is now every 15 min RTH (scheduler).
